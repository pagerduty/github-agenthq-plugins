...
If user says yes, write `~/.pagerduty/skill-backups/{skill-name}.json` (create the directory with `mkdir -p ~/.pagerduty/skill-backups` if it does not exist). Writing to the user's home keeps the backup out of any repository working tree, so the file cannot be accidentally committed:
```json
{
  "name": "{name}",
  "description": "{description}",
