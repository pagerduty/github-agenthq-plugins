...
**CRITICAL: verify PagerDuty Advance MCP is available before proceeding.**

Call `pagerduty-advance-list_skills_tool` with `agent_type: "sre"` and `scope: "user"` as a connectivity check.

If the call fails with a "tool not found", auth, or transport error, display the error and EXIT:

```
❌ PagerDuty Advance MCP is not available.

This skill requires the PagerDuty Advance MCP server to create and manage skills via API.
...
Do NOT proceed without MCP access — no silent degradation.

## API constraints

Skills are managed via the PagerDuty Advance MCP API with these constraints:

- **agent_type**: always `"sre"` (only the SRE Agent supports skills currently)
...
- **Credential-free** — Never embed API keys, tokens, passwords, or other secrets in skill instructions. Skill instructions are persisted in PagerDuty and surfaced to the SRE Agent at every invocation — anything in the body becomes a persistent leak. Reference credentials by env-var name (e.g., `$PAGERDUTY_API_KEY`, `$YOUR_SERVICE_TOKEN`) only.
...
**Your role:** translator between the user's natural language and agent-optimized skill definitions. The user describes what they want; YOU synthesize it into structured, agent-friendly instructions, descriptions, and examples.
...
Call `pagerduty-advance-list_skills_tool` with `agent_type: "sre"` and the chosen scope.
...
**3b. Fetch the current skill**

Call `pagerduty-advance-get_skill_tool` with `agent_type: "sre"`, the chosen scope, and `skill_name` to retrieve the full skill document.
...
Call `pagerduty-advance-list_skills_tool` with `agent_type: "sre"` and the chosen scope. Count results:
...
YOU synthesize everything into structured instructions. Translate the user's natural language into:
...
- **Error handling** (fail gracefully, report issues, continue where possible)
...
After finalizing, estimate token count: `len(instructions) / 4`. If > 4500 tokens (90% of 5000 limit), warn:
...
### Step 8: Suggest a name (CREATE) or show name (UPDATE)
...
- Check uniqueness: call `pagerduty-advance-list_skills_tool` with the chosen scope and verify the name doesn't already exist
- If the name exists in that scope, show the error and ask for a different name
- Same name can exist in both scopes, but must be unique within the chosen scope
...
Listen, then YOU translate into 2–3 clear, agent-friendly trigger conditions:
...
Display the complete skill structure:
...
If the user types "preview-json", show the exact API payload:
...
Call `pagerduty-advance-create_skill_tool` with parameters:
...
Call `pagerduty-advance-update_skill_tool` with parameters:
...
3. **Skill limit reached (CREATE only):**
   ...
   Existing skills: [list from pagerduty-advance-list_skills_tool]
...
4. **Validation errors (name format, length, tokens):**
   ...
   Return to the specific field that failed and ask the user to correct it.
...
5. **Authentication / API errors:**
   ...
   Check the API key and EA status, then try again.
   ```
...
If the user says yes, write `~/.pagerduty/skill-backups/{skill-name}.json` via `bash` (create the directory with `mkdir -p ~/.pagerduty/skill-backups` if it does not exist). Writing to the user's home keeps the backup out of any repository working tree, so the file cannot be accidentally committed:
...
When translating workflows into instructions, reference these integrations:
